...
# colnames(data)<- substr( colnames(data), 34, nchar(colnames(data))-10 )
...
run_deg<- function(first, second){
  first_indx<- which(names(conditions)%in%first)
  second_indx<- which(names(conditions)%in%second)
  for ( i in first_indx)
  {
    for( j in second_indx )
    {
      # get indices
      if ( i == j)
        next
...
run_deg("Full_CD8_V7" ,"fulldel_CD8_V7")
run_deg("CD3only_CD8_V7" ,"fulldel_CD8_V7")
run_deg("Costimonly_CD8_V7" ,"fulldel_CD8_V7")
